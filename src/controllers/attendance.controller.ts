...
export const getAttendanceSheet = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingAttendance = await prisma.attendance.findMany({
            where: {
                ...
                section: String(section) as 'MORNING' | 'AFTERNOON'
            }
        });
...
export const markAttendance = async (req: Request, res: Response): Promise<void> => {
    try {
        const { scheduleId, date, section, topic, records } = req.body as MarkAttendanceRequest;
