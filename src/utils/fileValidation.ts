...
export const sanitizeCSV = (data: any[]): any[] => {
...
export const validateMagicBytes = (buffer: Buffer, mimetype: string): boolean => {
    ...
    if ((mimetype === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') && hex.startsWith('504B0304')) return true;

    // CSV has no magic bytes (text), so validation relies on content parsing.
    if (mimetype === 'text/csv' || mimetype === 'application/vnd.ms-excel') return true;
