...
import { hash, decrypt } from '../../utils/encryption';

export class CRTService {
  async createBatch(data: any) {
    return await prisma.cRTBatch.create({
      data,
    });
  }

  async getBatches(studentId?: string) {
    const batches = await prisma.cRTBatch.findMany({
      where: {
        is_deleted: false,
  ...
  async previewBatch(minMarks: number, maxMarks: number) {
  ...
  async allocateBatch(
    ...
    allocations: Record<string, number>, // { CSE: 10, ECE: 5 }
    minMarks: number,
    maxMarks: number
  ) {
  ...
  async importStudentMarks(students: any[]) {
    ...
    for (const student of students) {
      try {
        await prisma.studentProfile.update({
          where: { roll_no_hash: hash(student.roll_no) },
          data: {
  ...
  async createSchedule(data: {
    ...
    return await prisma.cRTSchedule.create({
      data: {
        type: type as any,
        academic_year,
        name,
  ...
  async getSchedules(filters: { academic_year?: string, type?: string, studentId?: string, branch?: string }, page: number = 1, limit: number = 20) {
    ...
    const [schedules, total] = await Promise.all([
      ...
      prisma.cRTSchedule.count({ where: whereClause })
    ]);
  ...
  async deleteSchedule(scheduleId: string) {
    const schedule = await prisma.cRTSchedule.findUnique({ where: { id: scheduleId } });
    if (!schedule) throw new Error("Schedule not found");

    return await prisma.cRTSchedule.update({
      where: { id: scheduleId },
      data: { is_deleted: true }
    });
  }

  async getFacultySchedules(userId: string, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  private async calculateScheduleProgress(schedule: any) {
    ...
    const markedSessions = await prisma.attendance.groupBy({
      by: ['date', 'section'],
      where: {
  ...
  async getScheduleStudents(scheduleId: string) {
    ...
    const decryptedStudents = uniqueStudents.map((s: any) => ({
      ...s,
      roll_no: decrypt(s.roll_no)
    }));

    // Sort by roll number
    return decryptedStudents.sort((a: any, b: any) => a.roll_no.localeCompare(b.roll_no));
  }
  ...
  async getScheduleAnalytics(scheduleId: string) {
    const schedule = await prisma.cRTSchedule.findUnique({
      where: { id: scheduleId },
      include: {
        batches: true,
  ...
  async getAttendanceBySlot(scheduleId: string, date: string, section: 'MORNING' | 'AFTERNOON') {
    const attendanceDate = new Date(date);
