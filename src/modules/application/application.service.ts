...
export class ApplicationService {
  async apply(studentId: string, announcementId: string) {
  ...
  async getAll(filters: any, page: number = 1, limit: number = 50) {
    ...
    if (min_cgpa) studentFilter.cgpa = { gte: parseFloat(min_cgpa) };
  ...
  async bulkUpdateStatuses(companyName: string, updates: Array<{ roll_no: string, status: any }>) {
    ...
    for (const update of updates) {
      try {
        const rollHash = hash(update.roll_no);
