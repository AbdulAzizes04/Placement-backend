...
import { decrypt, hash } from '../utils/encryption';
...
interface ImportStudentRow {
    ...
    "Email": string;
    "CGPA": string;
    "Marks": string; // CRT Marks
    "roll_no"?: string; // For code logic
    "marks"?: string; // For code logic
}
...
interface BatchAllocationRequest {
    batch_name: string;
...
import { BulkStudentDto } from '../modules/student/dto/student.dto';

const studentService = new StudentService();

// --- Import Batches (CSV) ---
export const importBatches = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const bulkData: BulkStudentDto[] = students.map((s: any) => {
            // Helper to find value by case-insensitive key
            const getVal = (keys: string[]) => {
                const targetKey = Object.keys(s).find(k =>
                    keys.some(tk => k.trim().toLowerCase() === tk.toLowerCase())
                );
...
// --- Allocate Batches ---
export const allocateBatches = async (req: Request, res: Response): Promise<void> => {
    try {
        const { batches } = req.body as { batches: BatchAllocationRequest[] };
...
export const getBatches = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const whereClause: Prisma.CRTBatchWhereInput = { is_deleted: false };
        if (year) {
...
export const deleteAllBatches = async (req: Request, res: Response): Promise<void> => {
    try {
...
// --- Export Batch ---
export const exportBatch = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
...
export const unassignStudent = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const createBatchFromCSV = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const rollNoHashes = bulkData.map(s => hash(s.roll_no.toUpperCase()));
